...
func tsTypeToSchema(t *TSType) map[string]any {
	...
	case TSTypeLiteral:
		...
		if t.UseEnum {
			schema["enum"] = []any{t.ConstValue}
		} else {
			schema["const"] = t.ConstValue
		}
	...
	case TSTypeObject:
		schema["type"] = "object"
		if len(t.Properties) > 0 || t.HasProperties {
		...
		applyAdditionalProperties(schema, t)
		applyPatternProperties(schema, t)
	...
	case TSTypeUnion:
		if len(t.AnyOf) > 0 {
			anyOf := make([]any, len(t.AnyOf))
			for i, branch := range t.AnyOf {
				anyOf[i] = tsTypeToSchema(branch)
			}
			schema["anyOf"] = anyOf
		}
		// A union node may also carry a "type" field alongside anyOf
		// (e.g. when simple types + complex branches coexist).
		if t.Type != nil {
			schema["type"] = t.Type
		}
	...
	case TSTypeIntersection:
		if len(t.AllOf) > 0 {
			allOf := make([]any, len(t.AllOf))
			for i, branch := range t.AllOf {
	...
	if t.Format != "" && t.Kind != TSTypeSimple {
		schema["format"] = t.Format
	}

	// Apply annotations.
	applyAnnotations(schema, t.Annotations)
...
// applyAdditionalProperties sets additionalProperties on the schema.
func applyAdditionalProperties(schema map[string]any, t *TSType) {
...
// applyPatternProperties sets patternProperties on the schema.
func applyPatternProperties(schema map[string]any, t *TSType) {
...
// applyAnnotations merges TSAnnotations into a schema map.
func applyAnnotations(schema map[string]any, ann *TSAnnotations) {
	if ann == nil {
		return
	}
	if ann.Description != "" {
